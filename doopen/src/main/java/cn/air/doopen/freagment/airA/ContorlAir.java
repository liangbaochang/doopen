...
/**空气净化器A的控制界面；使用连接号控制；后面可能要被删除掉，因为这个方式已经被我们陶淘汰了*/
public class ContorlAir extends Fragment implements RadioName,OnClickListener{
	...
	private void setlistener() {
		Reduced_regulation.setProgress(0);
	...
	public void onClick(View v) {
		...
		case R.id.air_radio2://除静电
			...
			upDateValue(pattern=03);//05
			new GetDevListTask().execute();
			break;
		case R.id.air_radio0://除甲醛
			air_radio3.setChecked(false);air_radio4.setChecked(false);air_radio2.setChecked(false);air_radio0.setChecked(true);
			upDateValue(pattern=02);
			new GetDevListTask().execute();
			break;
		case R.id.air_share_imbtn_dev://
			...
			if (actions.equals(CN_XIEWEIMING_APP_TCPCOMM)) {
				...
				if (clientID != bundle.getInt("from")){
					...
					if (cmd==-127||cmd==-125) {
						...
						byte ligth_quey = buf.get();        // 1：昼夜模式，10：白天模式
						light=ligth_quey;
						Reduced_regulation.setProgress((int)light);
						MyLog.i(TAG, "灯的亮度=="+ligth_quey);
	...
	private void upDateValue(int contents){
		...
		buf.put((byte)contents);//模式控制
		buf.put((byte)light);//减压灯控制
		buf.put((byte)~(0x80+0x01+contents+light));//校验
	...
	public class GetDevListTask extends AsyncTask<Integer, Integer, Integer> {

		@Override
		protected Integer doInBackground(Integer... params) {
			return BindingListener.RefreshList(id_devid);
		}
		@Override
