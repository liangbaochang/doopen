...
public class TasksCompletedView extends View {
	...
	private void initVariable() {
		...
		mCirclePaint.setColor(0xff228cc8);
		mCirclePaint.setStyle(Paint.Style.FILL);

		mRingPaint = new Paint();
		mRingPaint.setAntiAlias(true);
		mRingPaint.setColor(0xff4cb749);
		mRingPaint.setStyle(Paint.Style.STROKE);
		mRingPaint.setStrokeWidth(mStrokeWidth);

		mRingPaint2 = new Paint();
		mRingPaint2.setAntiAlias(true);
		mRingPaint2.setColor(0xffe8e8e9);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mProgress >= 0 ) {
			...
			canvas.drawArc(oval, -90, ((float)100 / mTotalProgress) * 360, false, mRingPaint2); //
			canvas.drawArc(oval, -90, ((float)mProgress / mTotalProgress) * 360, false, mRingPaint); //
//			canvas.drawCircle(mXCenter, mYCenter, mRadius + mStrokeWidth / 2, mRingPaint2);
