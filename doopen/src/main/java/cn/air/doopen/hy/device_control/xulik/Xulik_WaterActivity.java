...
public class Xulik_WaterActivity extends Activity implements OnClickListener,RadioName{
	...
	private void initdata() {
		Intent intent1 = Xulik_WaterActivity.this.getIntent();
	...
	public void onClick(View v) {
		...
		case R.id.xulik_wash_btn:// 冲洗；
			if (Utils.isFastClick()) {
				shake();
				MyLog.i(TAG,"clientID="+ clientID);
				Water_control_tool.opendevwash((byte) 2, xulik_wash_btn, clientID, myBinder);
				//				Water_control_tool.opendevwash(23,(byte) 2, xulik_wash_btn, clientID, myBinder);
				xulik_content1.startRippleAnimation();
				//				new GetDevListTaskid().execute();
		...
		case R.id.xulik_Switch_btn:// 开关；
			if (Utils.isFastClick()) {
				shake();
				Water_control_tool.opendevwash((byte) 1, xulik_Switch_btn, clientID, myBinder);
				//				Water_control_tool.opendevwash(23,(byte) 1, xulik_wash_btn, clientID, myBinder);
	...
	private void filter(String type,int CntVs,int SetVs,int indexs){
		Intent RO=new Intent(getApplicationContext(), Filter_details_Activity.class);
	...
	//获取最新的连接号；
	public class GetDevListTaskid extends AsyncTask<Integer, Integer, Integer> {
		@Override
